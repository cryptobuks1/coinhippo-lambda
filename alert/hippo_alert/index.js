...
exports.handler = async (event, context, callback) => {
  ...
  transactionsSorted = transactionsSorted && transactionsSorted.filter(x => x.from_address_name && x.to_address_name && (x.from_address_name.toLowerCase().indexOf('unknown owner ') < 0 || x.to_address_name.toLowerCase().indexOf('unknown owner ') < 0) && !(x.from.owner_type === 'exchange' && x.to.owner_type === 'exchange' && x.from_address_name === x.to_address_name));
